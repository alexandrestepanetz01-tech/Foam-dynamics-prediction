...
def plotting_configuration(cnf, y, data_matrix, vmin, vmax, name):
    ...
    # save figure
    file_name =  f'Images/NN/t{timestep}'
    os.makedirs(file_name, exist_ok=True)
    plt.savefig(f'{file_name}/Simu_{cnf}_{name}.png', bbox_inches='tight')
    plt.close()

def pearson_corr(y_true, y_pred):
    # transform the original function in a one compatible with sklearn
...
def NN(data):
    ...
    model = MLPRegressor(hidden_layer_sizes=(8, 8, ), activation='relu', batch_size=50)
    ...
    # for i in [56, 110, 165]:
        ...
        # plt.savefig(
        #     f'{folder_name}/t{timestep}/Pearson_{i}.png',
        #     bbox_inches='tight')
        # plt.close()
